...
public final class OniGamePlugin extends JavaPlugin implements Listener, CommandExecutor, TabCompleter {
    ...
    @Override public boolean onCommand(CommandSender sender,Command command,String label,String[] a){
        ...
        if(a[0].equalsIgnoreCase("finalsky")&&sender instanceof Player p){
            ...
            if(a[1].equalsIgnoreCase("status")){msg(p,"Custom Sky: "+(getConfig().getBoolean("final-crimson.blood-sky.optifine-custom-sky",true)?"&aON":"&cOFF")+" &f/ test-time=&e"+getConfig().getLong("final-crimson.blood-sky.optifine-test-player-time",13000L)+" &f/ weather=&e"+"CLEAR");return true;}
    ...
    // v0.40.13: ロビー=朝 / ゲーム=夜 / FINAL=Custom Sky専用時間を明確に分離。
    ...
    private void enforcePhaseTime(){
        ...
        if(state==GameState.WAITING)world.setTime(lobbyMorningTime());
            ...
            world.setTime(finalPhase ? Math.floorMod(getConfig().getLong("final-crimson.player-time",13000L),24000L) : gameNightTime());
            if(finalPhase){
                long finalTime=Math.floorMod(getConfig().getLong("final-crimson.player-time",13000L),24000L);
    ...
    private void startFinalCrimson(World world){
        ...
        world.setTime(Math.floorMod(getConfig().getLong("final-crimson.player-time",13000L),24000L));
        ...
        long playerTime=Math.floorMod(getConfig().getLong("final-crimson.player-time",13000L),24000L);
    ...
    private void startFinalSkyTest(Player p){
        ...
        long playerTime=Math.floorMod(getConfig().getLong("final-crimson.blood-sky.optifine-test-player-time",13000L),24000L);
        p.setPlayerTime(playerTime,false);
    ...
    private void startFinalEffectTest(Player p){
        ...
        long playerTime=Math.floorMod(getConfig().getLong("final-crimson.player-time",13000L),24000L);p.setPlayerTime(playerTime,false);
        p.setPlayerWeather(WeatherType.CLEAR);
